...
data<-read_csv("~/Documents/R/PacificbmuQAQC/NCRMP_BMU_Pacific2023_Complete_BRC.csv")
head(data)
...
plot7<- ggplot(data=data) + 
  geom_boxplot(aes(x=SubRegion, y=((PostCTGrazingRate+PostCTMacroboringRate)*-1)))+
  labs(y = bquote('Erosion Rate'~(cm^3/year)))+ geom_point(aes(x=SubRegion, y=((PostCTGrazingRate+PostCTMacroboringRate)*-1)), shape=16, size=3)
...
plot8<- ggplot(data=data) + 
  geom_boxplot(aes(x=SubRegion, y=(PostCTAccretionRate)))+
  labs(x="Region", y = bquote('Accretion Rate'~(cm^3/year)))+ geom_point(aes(x=SubRegion, y=(PostCTAccretionRate)), shape=16, size=3)
...
library(tidyr)
data2<-select(data,SubRegion,SiteName, tagNum, PostCTGrazing, PostCTMacroboring, PostCTAccretion)
...
bmubar = ggplot(data = data3, aes(x=SiteName, y=carbAmount, fill=bmuAttribute))+
  geom_bar(stat="identity", color="black",
           position = position_dodge())+
  labs(y=expression("Amount change (cm3)"), x="BMU Site")+
